...
def analyze_pdf_with_llm(text):
    ...
    try:
        ...
        1. Autores: Lista de nomes completos dos autores
        2. Assunto Principal: Área de estudo principal
        3. Termos-Chave: 3-5 termos técnicos essenciais
        4. Ano de Publicação: Ano (número)
        5. Summary: Resumo detalhado com introdução sobre os assuntos tratados (5-7 frases)
...
def create_search_json(metadata):
    ...
    input_busca = f"Procure informações sobre {metadata['assunto_principal']} relacionadas aos termos {', '.join(metadata['termos_chave'])} dos autores {', '.join(metadata['autores'])}"
    ...
    search_json = {
        ...
        "filters": {
            ...
            "author_names": metadata["autores"],
            "publication_year_min": metadata["ano_publicacao"],
    ...
    try:
        ...
        return {
            ...
            "filters": {
                ...
                "author_names": ["Autor não identificado"],
                "publication_year_min": datetime.now().year,
